...
function line(frame, y, color = [255, 255, 255], count = 18) { for (let letter = 0; letter < count; letter += 1) for (let row = 0; row < 5; row += 1) for (let column = 0; column < 4; column += 1) { const offset = ((y + row) * frame.width + 110 + letter * 9 + column) * 4; frame.data[offset] = color[0]; frame.data[offset + 1] = color[1]; frame.data[offset + 2] = color[2]; } }
test('rejects empty and uniform lower areas', () => {
  for (const fill of [0, 255]) { const frame = image(); frame.data.fill(fill); assert.equal(validateSubtitleCandidate(frame, { x: 0, y: 200, width: 400, height: 40 }).valid, false); }
});
...
test('accepts complete short subtitle shapes with different character counts', () => {
  for (const text of ['No.', 'Go!', 'Wait!']) {
    const frame = image(); line(frame, 200, [255, 255, 255], text.replace(/[^a-z]/gi, '').length); const result = validateSubtitleCandidate(frame, { x: 100, y: 195, width: text.replace(/[^a-z]/gi, '').length * 9 + 20, height: 16 });
