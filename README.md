...
Electron desktop overlay for reading English text from a selected screen area with OCR and translating it into the selected language.

Current package version: **0.2.3**. OCR uses the bundled English model locally; translation and dictionary context requests require network access.

The title bar uses a three-line menu and a settings gear, with localized tooltips. The Windows executable and application windows use the subtitle icon in `src/assets/`.

## Features

### Translate subtitles
- Choose the original English subtitle area directly from the main window, then select **Start translation**.
- The application checks for changed frames every 200 ms and translates accepted text into the selected language.
- **Stop translation** pauses scanning. **Change area** replaces the persistent subtitle region.
- **Menu (☰) → Read subtitle area once** performs one read of the saved region.
...
### Interface language
- **Settings → Appearance → Interface language** offers English (default), Russian, Simplified Chinese, Hindi and Spanish. Names appear in their own languages.
- The selection is saved and updates open application windows without restarting. Resetting settings restores English.

### Translation language
- **Settings (⚙) → Translation → Translation language** offers Russian, Simplified Chinese, Hindi, Spanish and English. The default translation target is Russian.
- Changing the interface language also selects that language for translation. You can then choose a different translation target independently.
- OCR reads English only. Choosing English as the target displays the original text without a translation request.
- Switching languages clears cached results and discards pending results for the previous language. Saved dictionary entries retain their translation language.
...
- The main window has minimize and quit controls. Drag the title bar or resize the window normally; size is saved. **Menu (☰) → Restore window size** restores size and position.
...
- **Menu (☰) → Dictionary** opens search, sorting, pronunciation, context examples, deletion and export to CSV or JSON.
- Pages contain up to 20 entries and scroll to accommodate long words or translations.
- **Menu (☰) → Review words** offers one pass through all saved words, with a progress indicator and completion state.
...
- Interface translations use shared catalogs for all five supported languages, with English as the fallback.
...
Download the EXE from the [latest release](https://github.com/Domalega/Subtitle-Translator-Overlay/releases/latest) and run it. Node.js and a separate OCR model download are not required. Release assets also include license documents and checksums.
...
For a portable Windows executable, run `npm run build`. The output is `dist/Subtitle-Translator-Overlay-0.2.3.exe`. Both build commands run verification first. The current configuration produces an unsigned portable Windows executable. After building, run `./scripts/prepare-release.ps1` in PowerShell to prepare the EXE and license documents in `dist/release/`.
...
- `src/assets/`: application icons.
- `resources/ocr/`: tracked English OCR model, copied into packaged resources.
- `licenses/`: third-party license texts and source provenance.
...
`src/` contains application code and interface assets; `resources/` contains the external OCR model used by that code. Both are needed to build from source. The portable EXE includes everything needed to run the application.
...
Subtitle Translator Overlay is licensed under the [MIT License](LICENSE).
The application includes third-party open-source components under their own
licenses. See [THIRD_PARTY_NOTICES.txt](THIRD_PARTY_NOTICES.txt), [licenses/](licenses/)
and the [OCR model provenance](resources/ocr/README.md).
Windows builds include these documents in their resources directory; GitHub
Releases also attach LICENSE, THIRD_PARTY_NOTICES.txt and licenses.zip.
